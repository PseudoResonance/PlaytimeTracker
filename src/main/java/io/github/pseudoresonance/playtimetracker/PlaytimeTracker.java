...
public class PlaytimeTracker {
	...
	public void connected(ClientConnectedToServerEvent event) {
		currentServerIp = "mp." + event.manager.getRemoteAddress().toString().toLowerCase().split("/")[0].trim().replaceAll("^\\.+", "").replaceAll("\\.+$", "");
		if (event.isLocal)
			currentServerIp = "sp." + Minecraft.getMinecraft().getIntegratedServer().worldServers[0].getSaveHandler().getWorldDirectory().getName().trim();
